...
const j = (r) => r.json();
const api = (p, opts = {}) => fetch(BASE + p, { headers: { 'content-type': 'application/json' }, ...opts });
const admin = (p, opts = {}) => api(p, { ...opts, headers: { 'content-type': 'application/json', 'x-admin-password': PW, ...(opts.headers || {}) } });
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const turn = (profileId, text) => api('/api/turn', { method: 'POST', body: JSON.stringify({ profileId, text }) }).then(j);
const inTray = async (pid, aid) => (await api(`/api/artifacts?profileId=${pid}`).then(j)).artifacts.some((a) => a.id === aid);
const newKid = async (name) => (await admin('/api/admin/profiles', { method: 'POST', body: JSON.stringify({ name, color: '#3b82f6', age: 6 }) }).then(j)).id;

async function waitHealth(timeout = 10000) {
  const t0 = Date.now();
  while (Date.now() - t0 < timeout) { try { const r = await fetch(BASE + '/api/health'); if (r.ok) return; } catch {} await sleep(200); }
...
before(async () => {
  for (const f of [DBP, DBP + '-journal', DBP + '-wal', DBP + '-shm']) { try { fs.rmSync(f); } catch {} }
  srv = spawn('node', ['--experimental-sqlite', 'server/index.js'], {
    cwd: ROOT, stdio: 'ignore',
    env: { ...process.env, WONDRY_DB: DBP, PORT: String(PORT), ANTHROPIC_API_KEY: '', ADMIN_PASSWORD: PW, PIPER_VOICES_DIR: path.join(os.tmpdir(), 'wondry-novoices') },
  });
  await waitHealth();
  const r = await admin('/api/admin/profiles', { method: 'POST', body: JSON.stringify({ name: 'TestKid', initials: 'TK', color: '#16b8a6', age: 6, reading_level: 'early reader' }) }).then(j);
  kidId = r.id;
});
after(() => {
...
test('per-kid theme defaults to light and round-trips through save', async () => {
  ...
  await admin('/api/admin/profiles', { method: 'POST', body: JSON.stringify({ id: kidId, name: 'TestKid', initials: 'TK', color: '#16b8a6', theme: 'dark' }) });
  assert.equal((await api('/api/profiles').then(j)).profiles.find((p) => p.id === kidId).theme, 'dark');
  await admin('/api/admin/profiles', { method: 'POST', body: JSON.stringify({ id: kidId, name: 'TestKid', initials: 'TK', color: '#16b8a6', theme: 'banana' }) });
  assert.equal((await api('/api/profiles').then(j)).profiles.find((p) => p.id === kidId).theme, 'light');
});
...
test('an explicit request builds a ready, self-contained, sandboxed page', async () => {
  ...
  assert.ok(res.artifact && res.artifact.id === res.artifactId, 'artifact object returned so the card renders below the reply');
...
test('websocket emits created and completed for a generation', async () => {
  ...
  ws.addEventListener('message', (e) => { try { types.push(JSON.parse(e.data).type); } catch {} });
...
test('tray seen-tracking clears the badge', async () => {
  ...
  await api(`/api/artifacts/${target.id}/seen`, { method: 'POST', body: JSON.stringify({ profileId: kidId }) });
...
test('parent authoring is held until toggled on for a child, and is reversible', async () => {
  const { artifactId } = await admin('/api/admin/author', { method: 'POST', body: JSON.stringify({ topic: 'a bedtime story about the moon' }) }).then(j);
  await pollReady(artifactId);
  assert.equal(await inTray(kidId, artifactId), false);
  await admin(`/api/admin/artifacts/${artifactId}/audience`, { method: 'POST', body: JSON.stringify({ profileId: kidId, on: true }) });
  assert.equal(await inTray(kidId, artifactId), true);
  await admin(`/api/admin/artifacts/${artifactId}/audience`, { method: 'POST', body: JSON.stringify({ profileId: kidId, on: false }) });
  assert.equal(await inTray(kidId, artifactId), false);
});

test('a page can be shared to a second child', async () => {
  const kid2 = await newKid('Sib');
  const { artifactId } = await admin('/api/admin/author', { method: 'POST', body: JSON.stringify({ topic: 'fun facts about clouds' }) }).then(j);
  await pollReady(artifactId);
  await admin(`/api/admin/artifacts/${artifactId}/audience`, { method: 'POST', body: JSON.stringify({ profileId: kidId, on: true }) });
  await admin(`/api/admin/artifacts/${artifactId}/audience`, { method: 'POST', body: JSON.stringify({ profileId: kid2, on: true }) });
  assert.equal(await inTray(kidId, artifactId), true);
  assert.equal(await inTray(kid2, artifactId), true);
});

test('deleting content removes it for everyone', async () => {
  const { artifactId } = await admin('/api/admin/author', { method: 'POST', body: JSON.stringify({ topic: 'how rainbows form' }) }).then(j);
  await pollReady(artifactId);
  await admin(`/api/admin/artifacts/${artifactId}/audience`, { method: 'POST', body: JSON.stringify({ profileId: kidId, on: true }) });
...
test('system prompt is editable and persists', async () => {
  ...
  await admin('/api/admin/config', { method: 'POST', body: JSON.stringify({ systemPrompt: c0.systemPrompt + '\n' + marker }) });
...
test('parent can author a reading lesson and recorded attempts feed the report', async () => {
  const { artifactId } = await admin('/api/admin/author-reading', { method: 'POST', body: JSON.stringify({ interest: 'dinosaurs', level: 3, profileId: kidId }) }).then(j);
  ...
  await api(`/api/content/${artifactId}/event`, { method: 'POST', body: JSON.stringify({
    profileId: kidId, event: { pageIndex: 0, lineIndex: 0, expected, transcript: expected, score: 1,
      perWord: expected.split(/\s+/).map((w) => ({ word: w, ok: true })) } }) });
  await api(`/api/content/${artifactId}/event`, { method: 'POST', body: JSON.stringify({
    profileId: kidId, event: { pageIndex: 0, lineIndex: 1, expected: 'the quick brown fox', transcript: 'the brown', score: 0.5,
      perWord: [{ word: 'the', ok: true }, { word: 'quick', ok: false }, { word: 'brown', ok: true }, { word: 'fox', ok: false }] } }) });
...
test('a flashcards request builds a declarative deck', async () => {
  ...
  assert.ok(fc.cards.every((c) => c.front && c.back), 'cards have front + back');
});
...
test('a memory-game request builds a native game with 6 distinct pairs', async () => {
  ...
  assert.ok(game.pairs.every((p) => p.emoji && p.label), 'each pair has emoji + label');
});
...
test('an explorable request builds a declarative scene with focusable nodes', async () => {
  ...
  assert.ok(scene.nodes.every((nd) => nd.label && nd.emoji), 'nodes have label + emoji');
});

test('presence ingress validates state and reflects it (greet-on-approach)', async () => {
  const bad = await api('/api/presence', { method: 'POST', body: JSON.stringify({ state: 'nope' }) });
  assert.equal(bad.status, 400, 'invalid state rejected');
  const ok = await api('/api/presence', { method: 'POST', body: JSON.stringify({ state: 'present' }) }).then(j);
...
test('content-types manifest lists registered types with renderers + create forms', async () => {
  ...
  for (const id of ['page', 'reading', 'flashcards', 'memory', 'explorable']) assert.ok(ids.includes(id), 'missing type ' + id);
...
test('generic admin authoring works for any content type', async () => {
  const { artifactId } = await admin('/api/admin/content', { method: 'POST', body: JSON.stringify({ typeId: 'memory', params: { theme: 'space' } }) }).then(j);
...
test('disabling a content type globally removes it from manifests + voice routing', async () => {
  await admin('/api/admin/content-types/flashcards', { method: 'POST', body: JSON.stringify({ enabled: false }) });
  ...
  await admin('/api/admin/content-types/flashcards', { method: 'POST', body: JSON.stringify({ enabled: true }) });
...
test('per-child disabled types are skipped in voice routing', async () => {
  ...
  await admin('/api/admin/profiles', { method: 'POST', body: JSON.stringify({ id: k, name: 'NoGames', initials: 'NG', color: '#3b82f6', age: 6, disabledTypes: ['memory'] }) });
