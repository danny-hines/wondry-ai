...
initSchema();
const { openaiBody, parseOpenaiText, usageTokens } =
  await import('../server/services/providers.js');
const { resolveProviderByName } = await import('../server/config.js');

test('usageTokens normalizes both Anthropic and OpenAI shapes', () => {
  assert.deepEqual(usageTokens({ usage: { input_tokens: 10, output_tokens: 5 } }), {
    inTok: 10,
    outTok: 5,
  });
  assert.deepEqual(usageTokens({ usage: { prompt_tokens: 8, completion_tokens: 3 } }), {
    inTok: 8,
    outTok: 3,
  });
...
test('openaiBody prepends system, sets the token param, and includes tools', () => {
  ...
  const reasoning = openaiBody(
    { model: 'o-next', max_tokens: 8000, token_param: 'max_completion_tokens' },
    { messages: [{ role: 'user', content: 'hi' }] },
  );
  assert.equal(reasoning.max_completion_tokens, 8000);
  assert.equal(reasoning.max_tokens, undefined);

  const withTools = openaiBody(p, {
