...
const reset = () => { db.exec('DELETE FROM artifacts'); setKV('content_richness', 'standard'); setKV('richness_daily_cap', '0'); };
const addOnDemand = (type = 'page') =>
  db.prepare('INSERT INTO artifacts (id,title,source,status,type,created_at) VALUES (?,?,?,?,?,?)')
...
test('on-demand degrades to the simplest tier past the daily cap — any type counts', () => {
  ...
  addOnDemand('page'); addOnDemand('explorable');     // 2 == cap → not over yet
...
test('parent generations and per-create overrides bypass the cap', () => {
  ...
  addOnDemand(); addOnDemand(); addOnDemand();        // well over the cap
