...
const { synthesize, listVoices, kokoroEnabled, kokoroVoices } = await import('../server/services/tts.js');
...
test('kokoro is enabled via KOKORO_URL and its voices are listed (prefixed)', () => {
  ...
  assert.ok(listVoices().some((v) => v.startsWith('kokoro:')), 'kokoro voices appear in the picker');
});
