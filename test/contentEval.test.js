...
const { initSchema, db, insertEval, listEvals, evalSummary, evaluatedTargetIds, recentAvatarReplies } = await import('../server/db.js');
initSchema();
...
test('insert + latest-per-target listing, worst-first', () => {
  insertEval({ kind: 'page', targetId: 'a1', label: 'volcanoes', scores: { accuracy: 5, age_fit: 4, engagement: 4, clarity: 5 }, overall: 4.5, verdict: 'solid', issues: [] });
  insertEval({ kind: 'page', targetId: 'a2', label: 'sharks', scores: { accuracy: 3, age_fit: 2, engagement: 2, clarity: 2 }, overall: 2, verdict: 'weak', issues: ['dry', 'wall of text'] });
  insertEval({ kind: 'page', targetId: 'a1', label: 'volcanoes', scores: { accuracy: 3, age_fit: 3, engagement: 3, clarity: 3 }, overall: 3, verdict: 'regressed', issues: [] });
...
test('chat evals carry prompt + response and their own dimensions', () => {
  insertEval({ kind: 'chat', targetId: 'q0', label: 'why is the sky blue?', prompt: 'why is the sky blue?', response: 'Because sunlight scatters!', scores: { accuracy: 5, age_fit: 5, helpfulness: 4, tone: 5 }, overall: 4.5 });
...
test('score clamp keeps values in 1..5', () => {
  insertEval({ kind: 'page', targetId: 'a3', scores: { accuracy: 0, age_fit: 3, engagement: 3, clarity: 3 }, overall: 9 });
...
test('safety flag persists into the summary', () => {
  insertEval({ kind: 'page', targetId: 'a4', scores: { accuracy: 3 }, overall: 3, safety_ok: false });
...
test('recentAvatarReplies pairs replies with the preceding kid turn and honors the cutoff', () => {
  db.prepare("INSERT INTO profiles (id,name,initials,color,created_at) VALUES ('p','Kid','K','#fff',1)").run();
  db.prepare("INSERT INTO conversations (id,profile_id,started_at,last_activity) VALUES ('c','p',1,1)").run();
  const m = (id, role, kind, text, ts) => db.prepare("INSERT INTO messages (id,conversation_id,profile_id,role,kind,text,safety_flag,created_at) VALUES (?,?,?,?,?,?,0,?)").run(id, 'c', 'p', role, kind, text, ts);
...
test('extractContent: HTML page strips <style>, keeps text + script', () => {
  fs.writeFileSync(artifactPath('a1'), '<html><head><style>.x{color:red}</style></head><body><h1>Volcanoes</h1><script>const lava=1</script></body></html>');
...
test('extractContent: structured type returns pretty JSON; missing → null', () => {
  fs.writeFileSync(contentPath('a2'), JSON.stringify({ title: 'Sharks', pages: [{ lines: ['Sharks swim.'] }] }));
