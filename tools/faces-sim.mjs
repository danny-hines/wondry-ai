...
const normalize = (v) => { let s = 0; for (const x of v) s += x * x; s = Math.sqrt(s) || 1; return v.map((x) => x / s); };
const baseVec = (name) => { const r = rng(hash(name)); return normalize(Array.from({ length: DIM }, () => r() - 0.5)); };
...
const post = (p, body, headers = { 'content-type': 'application/json' }) =>
  fetch(URL + p, { method: 'POST', headers, body: JSON.stringify(body) }).then((r) => r.json()).catch((e) => ({ error: String(e) }));

async function seed() {
  const cfg = await post('/api/admin/config', { facesEnabled: true }, { 'content-type': 'application/json', 'x-admin-password': PW });
  console.log(cfg.error ? `(could not auto-enable — turn on Familiar faces in the console)` : '✓ Familiar faces enabled');
  for (const p of PEOPLE) {
    const base = baseVec(p.name), thumb = pngDataUri(24, 24, p.color);
    for (let i = 0; i < 8; i++) await post('/api/faces/observe', { faces: [{ embedding: noisy(base), thumb, quality: 0.9, trackId: 'sim-' + p.name }] });
  ...
  console.log(`Then:  node tools/faces-sim.mjs walk <name>   (e.g. ${PEOPLE[0].name}) to make the kiosk switch.`);
}

async function walk(name) {
  const p = PEOPLE.find((x) => x.name.toLowerCase() === (name || '').toLowerCase());
  if (!p) { console.error(`Unknown person "${name}". Try: ${PEOPLE.map((x) => x.name).join(', ')}`); process.exit(1); }
  const out = await post('/api/faces/observe', { faces: [{ embedding: noisy(baseVec(p.name)), trackId: 'walkup' }] });
  if (out.error || out.enabled === false) return console.log('Faces are off — run `seed` first (or enable in the console).');
  if (out.identified?.length) console.log(`✓ Recognized ${p.name} (conf ${out.identified[0].confidence.toFixed(3)}). If the kiosk is on the idle screen, it should switch to that child now.`);
  else console.log(`${p.name} isn't enrolled yet — assign their cluster to a kid in the console, then try again.`);
}
...
else console.log('Usage: node tools/faces-sim.mjs <seed | walk <name> | list>   (app must be running; URL=' + URL + ')');
