...
import {
  ...
  runChatEvals,
  runChatHistoryEvals,
} from '../server/services/evalRunner.js';
...
const kind = has('chat')
  ? 'chat'
  : ['page', 'reading', 'chat'].includes(arg('kind'))
    ? arg('kind')
    : 'page';
...
function printSummary() {
  ...
  if (worst.length) {
    ...
    for (const e of worst)
      console.log(
        `  ${fmt(e.overall)}  ${String(e.label || '')
          .slice(0, 44)
          .padEnd(44)}  ${String(e.verdict || '').slice(0, 56)}`,
      );
  }
}
...
async function main() {
  ...
  if (!liveGenerationEnabled()) {
    console.error(
      'No API key set — the judge needs a live model (set ANTHROPIC_API_KEY in .env). Aborting.',
    );
    process.exit(1);
  }
  ...
  const onProgress = (p) =>
    process.stdout.write(
      `\r  ${p.done}/${p.total}  ${String(p.label || '')
        .slice(0, 50)
        .padEnd(50)}`,
    );
  const t0 = Date.now();
  let res;
  if (has('matrix')) {
    console.log('Generating + judging a page matrix…');
    res = await runMatrixEvals({ onProgress });
  } else if (kind === 'chat' && has('history')) {
    console.log('Judging real conversations since the chat prompt last changed…');
    res = await runChatHistoryEvals({ reeval: has('reeval'), onProgress });
  } else if (kind === 'chat') {
    console.log('Running + judging the conversation suite…');
    res = await runChatEvals({ onProgress });
  } else {
    console.log(`Judging ${has('reeval') ? 'all' : 'un-evaluated'} ${kind} content…`);
    res = await runContentEvals({
      kind,
      reeval: has('reeval'),
      limit: parseInt(arg('limit', '200'), 10),
