...
router.post('/artifacts/:id/seen', (req, res) => {
  ...
  db.prepare(
    `INSERT INTO engagement (artifact_id,profile_id,seen) VALUES (?,?,1)
              ON CONFLICT(artifact_id,profile_id) DO UPDATE SET seen=1`,
...
router.post('/artifacts/:id/opened', (req, res) => {
  const { profileId } = req.body || {};
  db.prepare(
    `INSERT INTO engagement (artifact_id,profile_id,seen,opened,opened_at) VALUES (?,?,1,1,?)
...
router.post('/artifacts/:id/finished', (req, res) => {
  const { profileId } = req.body || {};
  db.prepare(
    `INSERT INTO engagement (artifact_id,profile_id,seen,opened,finished) VALUES (?,?,1,1,1)
              ON CONFLICT(artifact_id,profile_id) DO UPDATE SET finished=1`,
  ).run(req.params.id, profileId);
  res.json({ ok: true });
});
...
router.post(
  '/stt',
  express.raw({ type: ['application/octet-stream', 'audio/*'], limit: '12mb' }),
  async (req, res) => {
    if (!sttAvailable()) {
      return res.json({
        text: '',
