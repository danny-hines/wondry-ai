...
import {
  ...
  runSummary,
  listRunEvals,
  faceClustersForConsole,
  setFaceClusterProfile,
  deleteFaceCluster,
  releaseFaceClustersForProfile,
} from '../db.js';
...
router.get('/evals', (req, res) => {
  ...
  const promptChangedAt =
    kind === 'chat' ? (latestPromptVersion('chat_system_prompt')?.created_at ?? null) : undefined;
  ...
  const latestRun = latest
    ? {
        ...
        prevSummary: prev ? runSummary(prev.id) : null,
        prevWhen: prev ? prev.created_at : null,
      }
    : null;
  res.json({
    kind,
...
router.post('/evals/run', (req, res) => {
  ...
  if (!liveGenerationEnabled())
    return res.status(400).json({ error: 'no API key set — the judge needs a live model' });
  ...
  recordEvalRun({
    ...
    kind: k,
    mode: runMode,
    promptKey: promptKeyForKind(k),
    promptHash: isBench || k === 'chat' ? currentPromptHash(k) : null,
  });
  ...
  const run = isBench
    ? k === 'chat'
      ? runChatEvals({ batch, onProgress })
      : k === 'reading'
        ? runReadingMatrixEvals({ batch, onProgress })
        : runMatrixEvals({ batch, concurrency: 3, onProgress })
    : k === 'chat'
      ? runChatHistoryEvals({ batch, reeval: !!reeval, limit: 100, onProgress })
...
router.post('/evals/suggest', async (req, res) => {
  if (!liveGenerationEnabled())
    return res.status(400).json({ error: 'no API key set — suggestions need a live model' });
  const kind = EVAL_KINDS.includes((req.body || {}).kind) ? req.body.kind : 'page';
  try {
    res.json(await suggestPromptImprovement(kind));
  } catch (e) {
...
router.get('/reading-report', (req, res) => {
  const kids = db
    .prepare('SELECT id,name,initials,color,reading_level FROM profiles ORDER BY name')
    .all();
...
router.post('/profiles', (req, res) => {
  ...
  if (id && db.prepare('SELECT 1 FROM profiles WHERE id=?').get(id)) {
    db.prepare(
      'UPDATE profiles SET name=?,initials=?,color=?,age=?,reading_level=?,voice=?,persona=?,theme=?,interests=?,disabled_types=COALESCE(?,disabled_types) WHERE id=?',
    ).run(
      name,
      initials,
      color,
  ...
  const newId = id || uid();
  db.prepare(
    'INSERT INTO profiles (id,name,initials,color,age,reading_level,voice,persona,theme,interests,disabled_types,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
  ).run(
    newId,
    name,
...
router.post('/faces/clusters/:id/assign', (req, res) => {
  ...
  if (!profileId || !db.prepare('SELECT 1 FROM profiles WHERE id=?').get(profileId))
    return res.status(400).json({ error: 'unknown profile' });
  setFaceClusterProfile(req.params.id, profileId, 'assigned');
  res.json({ ok: true });
});
router.post('/faces/clusters/:id/ignore', (req, res) => {
  setFaceClusterProfile(req.params.id, null, 'ignored');
  res.json({ ok: true });
});
router.post('/faces/clusters/:id/unassign', (req, res) => {
  setFaceClusterProfile(req.params.id, null, 'pending');
  res.json({ ok: true });
});
router.post('/faces/clusters/:id/delete', (req, res) => {
  deleteFaceCluster(req.params.id);
  res.json({ ok: true });
});
...
router.get('/prompt-history', (req, res) => {
  ...
  if (!Object.values(PROMPT_FIELDS).includes(key))
...
router.post('/config', (req, res) => {
  ...
  for (const [field, key] of Object.entries(PROMPT_FIELDS)) {
    ...
    if (typeof val === 'string') {
      setKV(key, val);
      addPromptVersion({ key, value: val, author });
    }
  }
  if (typeof richness === 'string' && (getRichness().tiers || {})[richness])
    setKV('content_richness', richness);
  if (cap !== undefined && cap !== null && cap !== '')
    setKV('richness_daily_cap', String(Math.max(0, parseInt(cap, 10) || 0)));
