...
function activeConversation(profileId) {
  ...
  if (recent) {
    db.prepare('UPDATE conversations SET last_activity=? WHERE id=?').run(now(), recent.id);
    return recent.id;
  }
  const id = uid();
  db.prepare(
    'INSERT INTO conversations (id,profile_id,started_at,last_activity) VALUES (?,?,?,?)',
...
function conversationHistory(convId) {
  return db
    ...
    .all(convId, HISTORY_TURNS)
    .reverse()
    .map((r) => ({ role: r.role === 'kid' ? 'user' : 'assistant', content: r.text }));
}
function addMessage(convId, profileId, role, text, kind = 'text', artifactId = null, flag = 0) {
  ...
  const id = uid();
  db.prepare(
    `INSERT INTO messages (id,conversation_id,profile_id,role,kind,text,artifact_id,safety_flag,created_at)
              VALUES (?,?,?,?,?,?,?,?,?)`,
  ).run(id, convId, profileId, role, kind, text, artifactId, flag, now());
  return id;
}
...
const looksDeflecting = (r) =>
  /\b(grown[- ]?up|ask (a|an|your) (grown|grown-up|adult|parent|teacher)|something (else|fun|different)|talk about something)\b/i.test(
    r || '',
  );

const isAffirmation = (t) =>
  /^(yes|yeah|yep|yup|ya|yass+|sure|ok|okay|okey|please|yes please|yes plz|do it|build it|make it|i do|i would|sounds good|let'?s do it)\b/i.test(
    (t || '').trim(),
  );
...
const PAGE_OFFER_RE =
  /\s*(?:do you want me to |would you like me to |would you like to |want me to |want to |should i |shall i |can i |let me |i can |i could |how about i )(?:make|build|create|show|do)\s+(?:you\s+)?(?:a|an|one|some)?\s*(?:cool |fun |neat |little )?(?:page|lesson|game|quiz|story|activity)\b[^.?!]*[.?!]\s*$/i;
...
function isLearnable(text) {
  ...
  if (
    /how('?s| is) it going|how are you|your name|who are you|good (morning|afternoon|evening)|thank|bye|love you|i'?m (good|fine|ok)/.test(
      p,
    )
  )
    return false;
  ...
  if (
    /^(that|it|this|those|these|them|one|stuff|what|which|who|whom|whose|where|when|why|how|anything|something|nothing|idk)$/i.test(
      topic,
    )
  )
    return false;
...
async function resolveTopic(text, history) {
  ...
  const isRef =
    ...
    t === 'something fun' ||
    t.length < 3 ||
    /^(that|it|this|those|these|them|one|stuff)$/i.test(t);
  if (!isRef) return t;
  try {
    const r = (
      (await runText('resolve', { system: RESOLVE_TOPIC_SYSTEM_PROMPT, prompt: text, history })) ||
      ''
    ).trim();
    const cleaned = r
      .split('\n')[0]
    ...
    const conversational =
      ...
      /[?!]/.test(cleaned) ||
      /\b(i|i'?d|i'?m|sorry|but|need|understand|happy|can'?t|cannot|could you|what kind)\b/i.test(
        cleaned,
      );
...
async function topicAppropriate(topic, profile) {
  try {
    const r = (
      (await runText('safety', {
        system: TOPIC_SAFETY_PROMPT(profile?.age),
        prompt: `Topic: ${topic}`,
      })) || ''
    )
      .trim()
    ...
    return true;
  } // fail open — chat deflection + output scan back this up
}
const TOPIC_DEFLECTION =
  "Hmm, that's a big topic to explore with a grown-up. Want me to make something fun instead — like animals, space, or a story?";
...
router.post('/turn', async (req, res) => {
  ...
  if (isAffirmation(text) && pend && now() - pend.at < OFFER_TTL) {
    ...
    return res.json(
      await startArtifactTurn(
        convId,
        profile,
        pend.topic,
        `Yay! Let me make you a page about ${pend.topic}!`,
      ),
    );
  }
  ...
  if (timerReq) {
    ...
    if (timerReq.action === 'cancel') {
      ...
      const reply = active.length
        ? 'Okay, I stopped your timer.'
        : "You don't have a timer running right now.";
  ...
  if (remReq) {
    ...
    if (remReq.action === 'cancel') {
      ...
      const reply = active.length
        ? 'Okay, I cancelled your reminder.'
  ...
  const offForKid = new Set(
    (profile.disabled_types || '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
  );
  ...
  try {
    ...
    intentObj = JSON.parse(await runText('intent', { system: sys, prompt: text, history })) || {
      intent: 'chat',
    };
  } catch {
    intentObj = {
      intent:
        /\b(make|build|show|create|draw) (me )?(a |an )?(page|game|lesson|quiz|story|activity|picture)/i.test(
          text,
        )
          ? 'artifact'
          : 'chat',
  ...
  if (intent === 'timer' && Number(intentObj.durationSeconds) > 0) {
    ...
    return respondTimer(
      ...
      convId,
      profileId,
      Number(intentObj.durationSeconds) * 1000,
      intentObj.label,
    );
  }
  if (intent === 'reminder') {
  ...
  if (intent === 'artifact' && isTypeEnabled('page') && !offForKid.has('page')) {
    ...
    return res.json(
      await startArtifactTurn(
        ...
        profile,
        topic,
        'Ooh, let me build you something cool about that!',
      ),
    );
  }
  ...
  let reply = stripPageOffer(
    maskProfanity(
      (
        (await runText('chat', { system: getChatSystemPrompt(profile), prompt: text, history })) ||
        ''
      ).trim(),
    ),
  );
