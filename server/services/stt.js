...
function runProc(file, args) {
  return new Promise((resolve, reject) => {
    let err = '',
      proc;
    try {
      proc = spawn(file, args, { stdio: ['ignore', 'ignore', 'pipe'] });
    } catch (e) {
      return reject(e);
    }
...
async function transcribeCli(buf, mime) {
  ...
  const cleanup = () => {
    for (const f of [inFile, wavFile, outBase + '.txt']) {
      try {
  ...
  try {
    ...
    let text = '';
    try {
      text = fs.readFileSync(outBase + '.txt', 'utf8').trim();
    } catch {}
