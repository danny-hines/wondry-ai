...
import {
  DEFAULT_CHAT_SYSTEM_PROMPT, DEFAULT_ARTIFACT_SYSTEM_PROMPT, DEFAULT_READING_SYSTEM_PROMPT,
} from './systemPrompt.js';

// eval kind -> the editable system prompt it grades (KV key + saveConfig field + default).
export const KIND_PROMPT = {
  page:    { key: 'artifact_system_prompt', field: 'systemPrompt',        def: DEFAULT_ARTIFACT_SYSTEM_PROMPT, what: 'generates a self-contained interactive HTML learning page for a young child' },
  reading: { key: 'reading_system_prompt',  field: 'readingSystemPrompt', def: DEFAULT_READING_SYSTEM_PROMPT,  what: 'generates a structured reading-practice lesson as strict JSON' },
  chat:    { key: 'chat_system_prompt',     field: 'chatSystemPrompt',    def: DEFAULT_CHAT_SYSTEM_PROMPT,     what: "produces the avatar's short spoken replies to a child" },
};

export function promptHash(s) { return crypto.createHash('sha1').update(String(s)).digest('hex').slice(0, 12); }
export function promptKeyForKind(kind) { return KIND_PROMPT[kind]?.key || null; }
...
function parseJson(raw) {
  try { const a = raw.indexOf('{'), b = raw.lastIndexOf('}'); return JSON.parse(a >= 0 && b > a ? raw.slice(a, b + 1) : raw); }
  catch { return null; }
}
...
export async function suggestPromptImprovement(kind) {
  ...
  const base = { kind, field: meta.field, currentPrompt: current, changed: false, confidence: 'medium', summary: '', rationale: '', revisedPrompt: '' };
  ...
  if (!run) return { ...base, state: 'no-run', summary: 'Run a benchmark under the current prompt first, then ask for a suggestion.' };
  if (run.prompt_hash !== curHash) return { ...base, state: 'stale', runWhen: run.created_at, summary: 'The prompt has changed since the last benchmark — re-run the benchmark so the suggestion reflects the current prompt.' };

  const summary = runSummary(run.id);
  const weakest = listRunEvals(kind, run.id, 300)
    .filter((e) => (e.overall != null && e.overall < 4) || (e.issues && e.issues.length) || !e.safety_ok)
    .slice(0, 14)
    .map((e) => ({ item: e.subject || e.label || e.title || e.target_id, overall: e.overall, scores: e.scores, safety_ok: !!e.safety_ok, verdict: e.verdict, issues: e.issues || [] }));
  ...
  const prompt = `CURRENT SYSTEM PROMPT (the one that ${meta.what}):\n"""\n${current}\n"""\n\n`
  ...
  let raw;
  try { raw = await runText('judge', { system: META_SYSTEM, prompt }); }
  catch (e) { throw new Error('suggestion model call failed: ' + (e?.message || e)); }
  ...
  return {
    ...base, state: 'ok', runWhen: run.created_at, changed,
