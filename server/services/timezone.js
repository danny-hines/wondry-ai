...
export function supportedTimezones() {
  try { return Intl.supportedValuesOf('timeZone'); } catch { return [detectedTimezone()]; }
}
...
export function partsInZone(epoch, tz = getTimezone()) {
  ...
  const p = {};
  for (const part of dtf.formatToParts(new Date(epoch))) if (part.type !== 'literal') p[part.type] = part.value;
  return {
    year: +p.year, month: +p.month, day: +p.day,
    hour: +p.hour % 24, minute: +p.minute, second: +p.second, weekday: p.weekday,
  };
}
...
export function zonedWallTimeToEpoch({ year, month, day, hour, minute, second = 0 }, tz = getTimezone()) {
...
function epochForLocalHM(hour24, minute, day, tz, from) {
  ...
  const mk = (utcDayKey) => {
    ...
    return zonedWallTimeToEpoch({ year: d.getUTCFullYear(), month: d.getUTCMonth() + 1, day: d.getUTCDate(), hour: hour24, minute }, tz);
  };
  let epoch = mk(baseUTC);
  if (epoch <= from) {
    // Time already passed for the chosen day → roll to the next valid occurrence.
    if (day === null) epoch = mk(baseUTC + 86400000);          // tomorrow same time
...
export function nextEpochForLocalTime({ hour12, minute = 0, meridiem = null, day = null }, tz = getTimezone(), from = Date.now()) {
  const to24 = (h, mer) => (mer === 'am' ? (h === 12 ? 0 : h) : (h === 12 ? 12 : h + 12));
  const hours = meridiem ? [to24(hour12, meridiem)] : [...new Set([to24(hour12, 'am'), to24(hour12, 'pm')])];
  const cands = hours.map((h24) => epochForLocalHM(h24, minute, day, tz, from)).filter((e) => e > from);
