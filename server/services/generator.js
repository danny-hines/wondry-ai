...
export function contentPath(id) {
  return path.join(ARTIFACT_DIR, `${id}.json`);
} // native/declarative types
export function getContent(id) {
  try {
    return JSON.parse(fs.readFileSync(contentPath(id), 'utf8'));
  } catch {
    return null;
  }
}
...
export async function createArtifact({
  ...
  return usageContext.run({ artifactId: id }, async () => {
    ...
    db.prepare(
      ...
      type.id,
      pl.title || type.label,
      pl.promptText ?? JSON.stringify(resolved),
      prof.id,
      source,
      'generating',
      pl.subject ?? null,
      pl.reading_level ?? prof.reading_level ?? null,
      pl.plan ?? '',
      pl.emoji || type.emoji || '✨',
      color,
      1,
    ...
    finishArtifact(id, type, resolved, prof, { source, richness }).catch((e) =>
      failArtifact(id, e.message),
    );
    return id;
  });
}
...
async function finishArtifact(id, type, params, profile, ctx = {}) {
  const { data, meta = {} } = await type.generate({
    params,
    profile,
    source: ctx.source,
  ...
  if (type.safetyScan) {
    ...
    if (safe && safe.verdict === 'block')
      throw new Error('Content failed safety scan: ' + (safe.reasons || []).join(', '));
  }
  ...
  db.prepare(
    `UPDATE artifacts SET status='ready', title=?, emoji=?, color=?, subject=?, reading_level=?, plan=?, ready_at=? WHERE id=?`,
  ).run(
    meta.title ?? type.label,
    meta.emoji ?? type.emoji,
    meta.color ?? (profile.color || type.defaultColor || '#8b5cf6'),
    meta.subject ?? null,
    meta.reading_level ?? profile.reading_level ?? null,
    meta.plan ?? '',
    now(),
    id,
  );
...
export const startReadingGeneration = ({
  profile,
  source = 'on_demand',
  audience,
  interest,
