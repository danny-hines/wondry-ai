...
import { createScheduleRow, getScheduleRow, setScheduleStatus, activeSchedules, now } from '../db.js';
...
export function publicSchedule(row) {
  if (!row) return null;
  return {
    id: row.id, kind: row.kind, label: row.label, message: row.message,
    duration_ms: row.duration_ms, fire_at: row.fire_at, recurrence: row.recurrence,
...
function arm(row) {
  ...
  if (row.fire_at - now() > MAX_DELAY) {
    handles.set(row.id, setTimeout(() => { handles.delete(row.id); arm(getScheduleRow(row.id) || row); }, MAX_DELAY));
    return;
  }
...
export function startTimer({ durationMs, label = null, createdBy = 'voice' }) {
  const row = createScheduleRow({ kind: 'timer', label, durationMs, fireAt: now() + durationMs, createdBy });
  arm(row);
...
export function startReminder({ fireAt, message = null, label = null, recurrence = null, createdBy = 'parent' }) {
  const row = createScheduleRow({ kind: 'reminder', message, label, fireAt, recurrence, createdBy });
  arm(row);
  emit('schedule.created', { schedule: publicSchedule(row) });
  return publicSchedule(row);
}
...
export function listActiveTimers() {
  return activeSchedules().filter((r) => r.kind === 'timer').map(publicSchedule);
}
...
export function initScheduler() {
  ...
  if (rows.length) console.log(`  Schedules: re-armed ${rows.length} pending${overdue ? ` (${overdue} overdue, firing now)` : ''}`);
}
