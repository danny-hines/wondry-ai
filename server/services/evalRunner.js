...
import { db, READING_LEVELS, evaluatedTargetIds, recentAvatarReplies, latestPromptVersion } from '../db.js';
...
export async function runContentEvals({ kind = 'page', reeval = false, limit = 50, concurrency = 4, batch = 'existing', onProgress } = {}) {
  let rows = db.prepare("SELECT * FROM artifacts WHERE status='ready' AND type=? ORDER BY created_at DESC").all(typeForKind(kind));
  if (!reeval) { const done = evaluatedTargetIds(kind); rows = rows.filter((r) => !done.has(r.id)); }
  rows = rows.slice(0, limit);
  let judged = 0, skipped = 0;
  await pool(rows, concurrency, async (a, idx) => {
    const r = await judgeArtifact(a, { batch });
    if (r) judged++; else skipped++;
    onProgress?.({ done: idx + 1, total: rows.length, judged, skipped, label: a.subject || a.title });
  });
  return { mode: 'existing', kind, total: rows.length, judged, skipped, batch };
}

const DEFAULT_SUBJECTS = [
  'volcanoes', 'the water cycle', 'fractions', 'dinosaurs', 'the solar system',
  'photosynthesis', 'sharks', 'ancient Egypt', 'simple machines', 'the human heart',
];
...
export async function runMatrixEvals({ subjects = DEFAULT_SUBJECTS, levels, concurrency = 3, batch, onProgress } = {}) {
  ...
  await pool(combos, concurrency, async (c, idx) => {
    let art = null;
    try {
      const id = await createArtifact({ typeId: 'page', params: { topic: c.subject }, profile: { ...NEUTRAL, reading_level: c.level }, source: 'eval' });
      art = await waitReady(id);
    } catch { /* generation threw */ }
    const r = art ? await judgeArtifact(art, { batch }) : null;
    if (r) judged++; else failed++;
    onProgress?.({ done: idx + 1, total: combos.length, judged, failed, label: `${c.subject} · ${c.level}` });
  });
...
export async function runChatEvals({ concurrency = 4, batch, onProgress } = {}) {
  ...
  await pool(items, concurrency, async (item, idx) => {
    let reply = '';
    try { reply = (await runText('chat', { system, prompt: item.prompt }) || '').trim(); } catch { /* generation failed */ }
    const r = reply ? await judgeChat({ targetId: `q${item.index}`, prompt: item.prompt, response: reply }, { batch }) : null;
    if (r) judged++; else failed++;
...
export async function runChatHistoryEvals({ limit = 50, reeval = false, concurrency = 4, batch, onProgress } = {}) {
  ...
  if (!reeval) { const done = evaluatedTargetIds('chat'); rows = rows.filter((m) => !done.has(m.id)); }
  rows = rows.slice(0, limit);
  let judged = 0, skipped = 0;
  await pool(rows, concurrency, async (m, idx) => {
    const r = await judgeChat({ targetId: m.id, prompt: m.prompt, response: m.response }, { batch });
    if (r) judged++; else skipped++;
