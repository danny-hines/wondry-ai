...
const SCENE_NOUNS =
  /\b(solar system|the planets|water cycle|life ?cycle|food chain|food web|rock cycle|the seasons)\b/i;
...
export default {
  ...
  createForm: [
    {
      ...
      label: 'Topic',
      type: 'text',
      placeholder: 'e.g. the solar system, the human body, the water cycle',
    },
  ],

  matchIntent: (text) => (wantsScene(text) ? { topic: sceneTopic(text) || undefined } : null),
  intentReply: (params) =>
    params.topic ? `Ooh, let's explore ${params.topic}!` : `Ooh, let's explore something cool!`,

  prepare: ({ params }) => ({
    topic: (params.topic && String(params.topic).trim()) || 'the solar system',
  }),
  ...
  async generate({ params, profile, source, richness }) {
    ...
    const prompt =
      `Create an explorable interactive diagram for a child named ${profile.name || 'friend'}, age ${profile.age || 7}, ` +
    ...
    const doc = normalizeDoc(raw, {
      title: titleCase(params.topic),
      emoji: pickEmoji(params.topic),
      subject: extractTopic(params.topic),
    });
    if (lite)
      for (const b of doc.blocks)
        if (b.type === 'scene') {
          if (b.center) delete b.center.icon;
          for (const n of b.nodes) delete n.icon;
        }
    if (!doc.blocks.some((b) => b.type === 'scene'))
      throw new Error('Explorable doc had no scene block');
    const safe = checkDeclarativeContent(collectText(doc), profile);
    if (safe.verdict === 'block')
      throw new Error('Explorable failed safety scan: ' + safe.reasons.join(', '));
    ...
    return {
      ...
      meta: {
        ...
        color: profile.color || '#0891b2',
        subject: doc.subject,
        plan: `Explore ${scene.nodes.length} things about ${doc.subject}.`,
      },
    };
  },
};
