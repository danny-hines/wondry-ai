...
const wantsFlashcards = (t) =>
  /\b(flash ?cards?|study (set|cards)|quiz me|practi[sc]e (my )?(words|vocab|spelling|facts))\b/i.test(
    (t || '').trim(),
  );
...
export default {
  ...
  createForm: [
    {
      ...
      label: 'Topic',
      type: 'text',
      placeholder: 'e.g. farm animals, shapes, addition',
    },
  ],

  matchIntent: (text) =>
    wantsFlashcards(text) ? { topic: flashcardsTopic(text) || undefined } : null,
  intentReply: (params) =>
    params.topic
      ? `Fun! Let's make flashcards about ${params.topic}!`
      : `Fun! Let's make some flashcards!`,

  prepare: ({ params }) => ({
    topic: (params.topic && String(params.topic).trim()) || 'fun facts',
  }),
  ...
  async generate({ params, profile }) {
    const base =
      `Create a flashcards study set for a child named ${profile.name || 'friend'}, age ${profile.age || 7}, ` +
      `reading level "${profile.reading_level || 'early reader'}". Topic: "${params.topic}".`;
    const norm = {
      title: `${titleCase(params.topic)} Flashcards`,
      emoji: pickEmoji(params.topic),
    ...
    if (safe.verdict === 'block')
      throw new Error('Flashcards failed safety scan: ' + safe.reasons.join(', '));
    if (!mediaAgenticEnabled()) await resolveDocImages(doc); // bake image requests -> local cached media
    const nCards = doc.blocks
      .filter((b) => b.type === 'flashcards')
      .reduce((s, b) => s + b.cards.length, 0);
    return {
      data: doc,
      meta: {
        title: doc.title,
        ...
        color: profile.color || '#0ea5e9',
        subject: doc.subject,
        plan: `${nCards} flashcards about ${doc.subject}.`,
      },
    };
  },
};
