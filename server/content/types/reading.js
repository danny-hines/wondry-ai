...
async function inferInterests(profileId) {
  ...
  try {
    ...
    const r = await runText('summarize', {
      system:
        'List 2-4 specific topics or characters this child seems interested in, comma-separated. Reply with ONLY the comma-separated list, no other words.',
      prompt: blob,
    });
    return (r || '')
      .replace(/^[^:]*:\s*/, '')
...
const isReadingRequest = (t) =>
  /\b(read (with|to) me|read me (a |an )?(story|book)|reading (practice|time|lesson|game)|practice (my )?reading|help me (to )?read|read(ing)? along|read aloud|(a |an )?(story|book) (that )?i can read|(story|book) to read|let'?s read|can i read|i (want|wanna|would like) to read|teach me to read)\b/i.test(
    (t || '').trim(),
  );
...
export default {
  ...
  createForm: [
    {
      ...
      type: 'text',
      placeholder: '(optional, e.g. dinosaurs)',
    },
    { key: 'level', label: 'Level', type: 'level' },
  ],

  matchIntent: (text) =>
    isReadingRequest(text) ? { interest: readingInterest(text) || undefined } : null,
  intentReply: (params) =>
    params.interest
  ...
  async prepare({ params, profile }) {
    const interests =
      params.interest ||
      mergeInterests([profile.interests, await inferInterests(profile.id)]) ||
      'a fun story';
  ...
  async generate({ params, profile }) {
    const lesson = await runReading({
      profile,
      interests: params.interests,
      level: params.level,
      system: getReadingSystemPrompt(),
    });
    const safe = checkReadingContent(lesson, profile);
    if (safe.verdict === 'block')
      throw new Error('Reading lesson failed safety scan: ' + safe.reasons.join(', '));
    return {
      data: lesson,
      meta: {
        title: lesson.title,
        emoji: lesson.emoji,
        color: profile.color || '#16a34a',
        subject: lesson.interest,
        reading_level: String(params.level),
  ...
  summary(profileId) {
    return readingSummary(profileId);
  },
};
