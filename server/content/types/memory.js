...
const FALLBACK = [['🍎', 'Apple'], ['⭐', 'Star'], ['🌙', 'Moon'], ['🐶', 'Dog'], ['🚗', 'Car'], ['🌳', 'Tree'], ['🐟', 'Fish'], ['🎈', 'Balloon']];
const oneEmoji = (s) => { const m = String(s || '').match(/\p{Extended_Pictographic}/u); return m ? m[0] : ''; };
...
function normalizeGame(obj, theme) {
  ...
  for (const p of (Array.isArray(obj && obj.pairs) ? obj.pairs : [])) {
    ...
    const label = String((p && p.label) || '').replace(/\s+/g, ' ').trim().slice(0, 24);
    if (!emoji || !label || seen.has(emoji)) continue;
    seen.add(emoji); pairs.push({ emoji, label });
    if (pairs.length === 6) break;
  }
  for (const [emoji, label] of FALLBACK) { if (pairs.length >= 6) break; if (!seen.has(emoji)) { seen.add(emoji); pairs.push({ emoji, label }); } }
  return {
...
const wantsMemory = (t) => /\b(memory game|matching game|match(ing)? cards|memory match|play (a )?memory|concentration game)\b/i.test((t || '').trim());
...
export default {
  ...
  createForm: [{ key: 'theme', label: 'Theme', type: 'text', placeholder: 'e.g. animals, space, food' }],

  matchIntent: (text) => (wantsMemory(text) ? { theme: memoryTheme(text) || undefined } : null),
  intentReply: (params) => (params.theme ? `Yes! A ${params.theme} memory game coming up!` : `Yes! Let's play a memory game!`),
  ...
  async generate({ params, profile }) {
    ...
    return {
      data: game,
      meta: { title: game.title, emoji: game.emoji, color: profile.color || '#7c3aed', subject: params.theme, plan: `Match ${game.pairs.length} pairs!` },
    };
  },
  ...
  recordEvent({ artifactId, profileId, event }) {
    const e = event || {};
    recordProgressEvent({ artifactId, profileId, type: 'memory', kind: 'win', value: typeof e.score === 'number' ? e.score : 1, data: { moves: e.moves, pairs: e.pairs } });
  },
  summary(profileId) {
