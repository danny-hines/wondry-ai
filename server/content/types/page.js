...
export default {
  ...
  createForm: [
    {
      ...
      type: 'textarea',
      placeholder:
        'e.g. A gentle bedtime story about the moon, calm and short, with a counting game at the end.',
    },
  ],
  ...
  async plan({ params }) {
    ...
    try {
      const j = JSON.parse(
        await runText('plan', { system: PLAN_SYSTEM_PROMPT, prompt: `Topic: ${topic}` }),
      );
      return {
        title: j.title || titleCase(extractTopic(topic)),
        emoji: j.emoji || pickEmoji(topic),
        plan: j.plan || '',
        subject: extractTopic(topic),
        promptText: topic,
      };
    } catch {
      return {
        title: titleCase(extractTopic(topic)),
  ...
  async generate({ params, profile, source, richness }) {
    ...
    const r = await runArtifact({
      topic: params.topic,
      profile,
      system: getArtifactSystemPrompt(),
      tier,
    });
    const safe = checkOutputHTML(r.html, profile);
    if (safe.verdict === 'block')
      throw new Error('Output failed safety scan: ' + safe.reasons.join(', '));
    return {
      data: r.html,
      meta: {
        title: r.title,
