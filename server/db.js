...
export function initSchema() {
    ...
    if (!names.includes('_schedules_migrate')) {
      if (names.includes('timers') && !names.includes('schedules')) {
        ...
        if (cols.includes('profile_id'))
          db.exec('DROP TABLE IF EXISTS timers'); // ancient per-child → throwaway
        else
          db.exec(
            'DROP INDEX IF EXISTS idx_timers_status; ALTER TABLE timers RENAME TO _schedules_migrate',
          );
        ...
        db.exec(
          'DROP INDEX IF EXISTS idx_schedules_status; ALTER TABLE schedules RENAME TO _schedules_migrate',
        );
      }
    }
  } catch {
    /* no timers/schedules table yet */
  }
  ...
  try {
    db.exec('DROP TABLE IF EXISTS content_evals');
  } catch {
    /* none */
  }
  db.exec(`
  ...
  try {
    const has = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='_schedules_migrate'")
      .get();
    if (has) {
      const src = new Set(
        db
          .prepare('PRAGMA table_info(_schedules_migrate)')
          .all()
          .map((c) => c.name),
      );
      const dest = [
  ...
  if (aud === 0 && arts > 0) {
    const rows = db
      .prepare('SELECT id, profile_id FROM artifacts WHERE published=1 AND profile_id IS NOT NULL')
      .all();
    const ins = db.prepare(
      'INSERT OR IGNORE INTO artifact_audience (artifact_id,profile_id,created_at) VALUES (?,?,?)',
    );
...
export function setAudience(artifactId, profileId, on) {
  if (on) {
    db.prepare(
      'INSERT OR IGNORE INTO artifact_audience (artifact_id,profile_id,created_at) VALUES (?,?,?)',
    ).run(artifactId, profileId, now());
    db.prepare('INSERT OR IGNORE INTO engagement (artifact_id,profile_id,seen) VALUES (?,?,0)').run(
      artifactId,
      profileId,
    );
  } else {
    db.prepare('DELETE FROM artifact_audience WHERE artifact_id=? AND profile_id=?').run(
      artifactId,
      profileId,
    );
    db.prepare('DELETE FROM engagement WHERE artifact_id=? AND profile_id=?').run(
      artifactId,
      profileId,
    );
  }
...
export function audienceFor(artifactId) {
  return db
    .prepare('SELECT profile_id FROM artifact_audience WHERE artifact_id=?')
    .all(artifactId)
    .map((r) => r.profile_id);
}
...
export function insertMedia(m) {
  db.prepare(
    `INSERT INTO media (id,source,query,mime,ext,alt,credit,license,source_url,bytes,created_at)
              VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
  ).run(
    m.id,
    m.source ?? null,
...
export function recordUsage({
  ...
  db.prepare(
    `INSERT INTO api_usage (id,task,model,artifact_id,input_tokens,output_tokens,cost_usd,created_at)
              VALUES (?,?,?,?,?,?,?,?)`,
  ).run(
    uid(),
    task ?? null,
...
export function costByArtifact(ids) {
  ...
  const rows = db
    .prepare(
      `SELECT artifact_id, COALESCE(SUM(cost_usd),0) AS cost
    FROM api_usage WHERE artifact_id IN (${ids.map(() => '?').join(',')}) GROUP BY artifact_id`,
    )
    .all(...ids);
...
export function usageSince(ts) {
  return db
    ...
    COALESCE(SUM(output_tokens),0) AS outTok, COUNT(*) AS n FROM api_usage WHERE created_at >= ?`,
    )
    .get(ts);
}
export function usageByModel(ts) {
  return db
    .prepare(
      `SELECT model, COALESCE(SUM(cost_usd),0) AS cost, COUNT(*) AS n
    FROM api_usage WHERE created_at >= ? GROUP BY model ORDER BY cost DESC`,
    )
    .all(ts);
}
...
export function createFaceCluster(centroid) {
  const id = uid();
  db.prepare(
    `INSERT INTO face_clusters (id,status,n,centroid,created_at,updated_at) VALUES (?,?,?,?,?,?)`,
  ).run(id, 'pending', 1, JSON.stringify(centroid), now(), now());
  return id;
}
export function updateFaceClusterCentroid(id, centroid, n) {
  db.prepare(`UPDATE face_clusters SET centroid=?, n=?, updated_at=? WHERE id=?`).run(
...
export function insertFaceSample({ clusterId, embedding, thumb, quality = 0 }) {
  const id = uid();
  db.prepare(
    `INSERT INTO face_samples (id,cluster_id,embedding,thumb,quality,created_at) VALUES (?,?,?,?,?,?)`,
  ).run(
    id,
    clusterId ?? null,
...
export function faceClusterCentroids() {
  return db
    .prepare(`SELECT id, centroid, n FROM face_clusters WHERE status != 'ignored'`)
    .all()
...
export function assignedFaceGalleries() {
  return db
    .prepare(
      `SELECT id, profile_id, centroid FROM face_clusters WHERE status='assigned' AND profile_id IS NOT NULL`,
    )
    .all()
...
export function faceClustersForConsole(minSamples = 1) {
  const rows = db
    .prepare(
      `SELECT id, profile_id, status, n, created_at, updated_at FROM face_clusters ORDER BY updated_at DESC`,
    )
    .all();
  return rows
    .map((r) => ({
      ...
      status: r.status,
      count: clusterSampleCount(r.id),
      thumbs: clusterThumbs(r.id, 6),
...
export function clusterThumbs(clusterId, limit = 6) {
  return db
    .prepare(
      `SELECT thumb FROM face_samples WHERE cluster_id=? AND thumb IS NOT NULL ORDER BY quality DESC, created_at DESC LIMIT ?`,
    )
    .all(clusterId, limit)
    .map((r) => r.thumb);
}
...
export function trimClusterSamples(clusterId, max) {
  const extra = db
    .prepare(
      `SELECT id FROM face_samples WHERE cluster_id=? ORDER BY quality DESC, created_at DESC LIMIT -1 OFFSET ?`,
    )
    .all(clusterId, max)
    .map((r) => r.id);
  if (extra.length)
    db.prepare(`DELETE FROM face_samples WHERE id IN (${extra.map(() => '?').join(',')})`).run(
      ...extra,
    );
}
export function setFaceClusterProfile(id, profileId, status) {
  db.prepare(`UPDATE face_clusters SET profile_id=?, status=?, updated_at=? WHERE id=?`).run(
    profileId ?? null,
    status,
    now(),
    id,
...
export function releaseFaceClustersForProfile(profileId) {
  db.prepare(
    `UPDATE face_clusters SET profile_id=NULL, status='pending', updated_at=? WHERE profile_id=?`,
  ).run(now(), profileId);
}
export function faceEnrollmentCounts() {
  return Object.fromEntries(
    db
      .prepare(
        `SELECT profile_id, COUNT(*) AS n FROM face_clusters WHERE status='assigned' AND profile_id IS NOT NULL GROUP BY profile_id`,
      )
      .all()
      .map((r) => [r.profile_id, r.n]),
  );
}
...
export function createScheduleRow({
  ...
  const id = uid();
  db.prepare(
    `INSERT INTO schedules (id,kind,label,message,duration_ms,fire_at,recurrence,status,created_by,created_at)
              VALUES (?,?,?,?,?,?,?, 'pending', ?, ?)`,
  ).run(
    id,
    kind,
...
export function rescheduleRow(id, fireAt) {
  db.prepare("UPDATE schedules SET fire_at=?, status='pending', fired_at=NULL WHERE id=?").run(
    Math.round(fireAt),
    id,
  );
  return getScheduleRow(id);
...
const evalNum = (v) =>
  typeof v === 'number' && Number.isFinite(v) ? Math.max(1, Math.min(5, v)) : null;
export function insertEval(e) {
  ...
  db.prepare(
    ...
    scores,
    evalNum(e.overall),
    e.safety_ok === false ? 0 : 1,
    e.verdict ?? null,
    e.issues ? JSON.stringify(e.issues) : null,
    e.raw ?? null,
    now(),
  );
  return id;
...
export function evaluatedTargetIds(kind) {
  return new Set(
    db
      .prepare('SELECT DISTINCT target_id FROM evals WHERE kind=?')
      .all(kind)
      .map((r) => r.target_id),
  );
}
...
export function listEvals(kind, limit = 300) {
  ...
  return db
    .prepare(
      `SELECT ${cols} FROM evals e ${join}
    WHERE e.kind=? AND ${LATEST_PER_TARGET} ORDER BY e.overall ASC, e.created_at DESC LIMIT ?`,
    )
    .all(kind, limit)
    .map((r) => ({
      ...r,
      scores: r.scores ? safeParse(r.scores) : {},
      issues: r.issues ? safeParse(r.issues) : [],
    }));
}
...
function summarize(rows) {
  ...
  for (const r of rows)
    for (const [k, v] of Object.entries(r.scores || {})) {
      if (v != null) {
        (acc[k] = acc[k] || { sum: 0, n: 0 }).sum += v;
        acc[k].n++;
      }
    }
  const dims = Object.fromEntries(
    Object.entries(acc).map(([k, d]) => [k, d.n ? d.sum / d.n : null]),
  );
  return {
    n,
    overall: n ? rows.reduce((s, r) => s + (r.overall || 0), 0) / n : null,
    dims,
    safetyConcerns: rows.filter((r) => !r.safety_ok).length,
  };
}
// All-time snapshot: averages over the latest eval per target of a kind.
export function evalSummary(kind) {
  const rows = db
    .prepare(`SELECT e.* FROM evals e WHERE e.kind=? AND ${LATEST_PER_TARGET}`)
    .all(kind)
...
export function runSummary(batch) {
  const rows = db
    .prepare('SELECT overall, scores, safety_ok FROM evals WHERE batch=?')
    .all(batch)
    .map((r) => ({ ...r, scores: r.scores ? safeParse(r.scores) : {} }));
  return summarize(rows);
}
...
export function listRunEvals(kind, batch, limit = 300) {
  ...
  return db
    .prepare(
      `SELECT ${cols} FROM evals e ${join} WHERE e.kind=? AND e.batch=? ORDER BY e.overall ASC, e.created_at DESC LIMIT ?`,
    )
    .all(kind, batch, limit)
    .map((r) => ({
      ...r,
      scores: r.scores ? safeParse(r.scores) : {},
      issues: r.issues ? safeParse(r.issues) : [],
...
export function recordEvalRun({ id, kind, mode, promptKey = null, promptHash = null }) {
  db.prepare(
    'INSERT OR IGNORE INTO eval_runs (id,kind,mode,prompt_key,prompt_hash,created_at) VALUES (?,?,?,?,?,?)',
  ).run(id, kind, mode, promptKey, promptHash, now());
  return id;
}
export function evalRuns(kind, limit = 50) {
  return db
    .prepare('SELECT * FROM eval_runs WHERE kind=? ORDER BY created_at DESC, id DESC LIMIT ?')
    .all(kind, limit);
}
...
export function latestPromptVersion(key) {
  return db
    .prepare(
      'SELECT * FROM prompt_versions WHERE prompt_key=? ORDER BY created_at DESC, id DESC LIMIT 1',
    )
    .get(key);
}
...
export function addPromptVersion({ key, value, author = 'parent', note = null }) {
  ...
  const id = uid();
  db.prepare(
    'INSERT INTO prompt_versions (id,prompt_key,value,author,note,created_at) VALUES (?,?,?,?,?,?)',
...
export function promptVersions(key, limit = 50) {
  return db
    .prepare(
      'SELECT id,prompt_key,value,author,note,created_at FROM prompt_versions WHERE prompt_key=? ORDER BY created_at DESC, id DESC LIMIT ?',
    )
    .all(key, limit);
}
...
export function recordProgressEvent({
  ...
  const id = uid();
  db.prepare(
    `INSERT INTO progress_events (id,artifact_id,profile_id,type,kind,value,data,created_at)
              VALUES (?,?,?,?,?,?,?,?)`,
  ).run(
    id,
    artifactId || null,
...
export function progressEvents(profileId, type, limit = 400) {
  return db
    .prepare(
      'SELECT value, data, created_at FROM progress_events WHERE profile_id=? AND type=? ORDER BY created_at DESC LIMIT ?',
    )
...
export function readingSummary(profileId, recentWindow = 40) {
  ...
  const missWords = Object.entries(miss)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 12)
    .map(([word, n]) => ({ word, n }));
