...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    a.ok().then((ok) => {
      if (ok) setApi(a);
      else sessionStorage.removeItem('imag_pw');
    });
  }, []);
  useEffect(() => {
    if (api)
      api
        .config()
        .then((c) => setLive(c.liveGeneration))
        .catch(() => {});
  }, [api]);
  ...
  const doLogin = async () => {
    ...
    if (r.ok) {
      sessionStorage.setItem('imag_pw', pwInput);
      setApi(new AdminApi(pwInput));
      ...
      <div className="admin">
        <div className="card login">
          ...
          <div className="row" style={{ justifyContent: 'center' }}>
            ...
            />
            <button className="act" onClick={doLogin}>
              Enter
            </button>
          </div>
      ...
      <div className="admin">
        <header>
          ...
          <span className={`live ${live ? 'on' : ''}`}>
            generation: {live ? 'LIVE (Claude)' : 'mock'}
          </span>
        </header>
        <nav>
          {tabs.map(([to, label]) => (
