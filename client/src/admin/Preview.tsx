        ...
        if (onProgress)
          u.onboundary = (e) => {
            if (my === tokenRef.current)
              onProgress(Math.min(1, (e.charIndex || 0) / Math.max(1, u.text.length)));
          };
        const done = () => {
          if (my === tokenRef.current) setSpeakingId(null);
    ...
    return (
      <div className="preview-page">
        ...
        <div className="preview-stage">
          <p style={{ padding: 24 }}>Couldn't load this page.</p>
        </div>
      </div>
    );
  if (!art)
    return (
      <div className="preview-page">
        <div className="preview-head">Preview</div>
        <div className="preview-stage">
          <p style={{ padding: 24 }}>Loading preview…</p>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      <div className="preview-head">
        ...
        <span className="tag">
          {art.status === 'ready' ? `${art.type || 'page'} · preview` : `not ready (${art.status})`}
        </span>
      </div>
      <div className="preview-stage">
        {art.status !== 'ready' ? (
