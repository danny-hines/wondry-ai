...
function wordEnds(words: string[]): number[] {
  ...
  let acc = 0;
  return words.map((w) => { acc += w.length + 1; return acc / total; });
}
...
export default function Reader({ artifactId, profile, speak, setMood }: ReaderProps) {
  ...
  useEffect(() => {
    let live = true;
    getContent<ReadingLesson>(artifactId).then((l) => { if (live) setLesson(l); }).catch(() => { if (live) setErr(true); });
    return () => { live = false; genRef.current++; sessionRef.current?.stop(); };
  }, [artifactId]);
  useEffect(() => () => { sessionRef.current?.stop(); }, []); // stop mic on unmount
  ...
  const resetLine = () => {
    ...
    setWordHi(-1); setCursor(0); setLiveMarks([]); setFlash(false); setMarks(null); setFeedback(''); setPhase('idle');
  };
  ...
  const avatarReadLine = useCallback(async () => {
    ...
    sessionRef.current?.stop(); sessionRef.current = null;
    setPhase('avatar'); setMarks(null); setFeedback('');
  ...
  const startLive = useCallback(() => {
    if (!line) return;
    if (!canStream) { listenOnce(); return; }   // fallback for non-streaming engines
    const my = ++genRef.current;
    const expN = words.map(normWord);
    const heard = words.map(() => false);
    let cur = 0, done = false, hinted = false, lastText = '';
    setPhase('listen'); setMarks(null); setFeedback(''); setWordHi(-1);
    setLiveMarks(heard.slice()); setCursor(0);
    setMood('listening');
    ...
    const finish = () => {
      ...
      setMarks(perWord); setPhase('graded');
      const msg = feedbackFor(score, profile?.name || undefined);
      setFeedback(msg); speak(msg, profileId, 'reader', voice);
      postContentEvent(artifactId, profileId || '', { pageIndex: page, lineIndex: lineIdx, expected: line, transcript: lastText || '(live)', score, perWord });
      setTimeout(() => { if (my === genRef.current) advance(); }, score >= 0.6 ? 1400 : 2400);
    };
    ...
    const onText = (text: string) => {
      ...
      if (cz > cur) { cur = cz; hinted = false; setFlash(false); }
      setCursor(cur);
      if (cur >= expN.length) { finish(); return; }
      // Stuck on the current word? gentle red pulse + speak it once as a hint.
      if (trailingUnmatched >= 2 && !hinted) {
        hinted = true; setFlash(true);
        setTimeout(() => { if (my === genRef.current) setFlash(false); }, 800);
  ...
  const listenOnce = useCallback(async () => {
    const my = ++genRef.current;
    setPhase('listen'); setMarks(null); setFeedback(''); setMood('listening');
    let transcript = '';
    try { transcript = (await sttRef.current!.listen().result).transcript; } catch {}
    setMood('idle');
    if (my !== genRef.current) return;
    if (!transcript.trim()) {
      const msg = "I didn't catch that — let's try together!";
      setPhase('graded'); setMarks(null); setFeedback(msg); speak(msg, profileId, 'reader', voice);
      return;
    }
    ...
    const msg = feedbackFor(score, profile?.name || undefined);
    setFeedback(msg); speak(msg, profileId, 'reader', voice);
    postContentEvent(artifactId, profileId || '', { pageIndex: page, lineIndex: lineIdx, expected: line, transcript, score, perWord });
    if (score >= 0.75) setTimeout(() => { if (my === genRef.current) advance(); }, 1500);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const advance = useCallback(() => {
    ...
    if (lineIdx < last) { setLineIdx(lineIdx + 1); return; }
    if (page < lesson.pages.length - 1) { setPage(page + 1); setLineIdx(0); return; }
    setPhase('done');
    if (profileId) markEngagement(artifactId, 'finished', profileId);
    speak(`You finished the whole story! I'm so proud of you${profile?.name ? ', ' + profile.name : ''}!`, profileId, 'reader', voice);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (echo && autoReadRef.current !== key) { autoReadRef.current = key; avatarReadLine(); }
  }, [lesson, page, lineIdx, echo, phase, avatarReadLine]);

  if (err) return <div className="reader"><div className="reader-msg">Couldn't load this story. Try again!</div></div>;
  if (!lesson) return <div className="reader"><div className="reader-msg">📖 Opening your story…</div></div>;
  ...
  if (phase === 'done') {
    return (
      <div className="reader done">
        ...
        <button className="rbtn primary" onClick={() => { setPage(0); setLineIdx(0); autoReadRef.current = ''; resetLine(); }}>Read it again ↺</button>
      </div>
    );
  }
  ...
  return (
    <div className="reader" style={{ ['--user' as any]: profile?.color || '#16b8a6' }}>
      <div className="reader-bar">
        <span className="reader-title">{lesson.emoji} {lesson.title}</span>
        <span className="reader-prog"><span className="reader-prog-fill" style={{ width: `${(doneLines / totalLines) * 100}%` }} /></span>
            ...
            return (
              <p key={i} className={`rline${isCur ? ' current' : ''}`}>
                {lw.map((w, wi) => {
                  ...
                  if (isCur && phase === 'graded' && marks && marks[wi]) cls += marks[wi].ok ? ' ok' : ' miss';
      ...
      {phase === 'listen' && <div className="reader-hint">Read it out loud — I'm listening! 👂</div>}

      <div className="reader-controls">
        <button className="rbtn" disabled={phase === 'avatar' || phase === 'listen'} onClick={avatarReadLine}>🔊 Hear it</button>
        {phase === 'listen'
          ? <button className="rbtn live" onClick={() => finishRef.current?.()}>✓ Done</button>
          : <button className="rbtn primary" disabled={phase === 'avatar'} onClick={startLive}>🎤 My turn</button>}
        {phase === 'graded' && <button className="rbtn" onClick={advance}>Next →</button>}
      </div>
    </div>
  );
}
