...
export function ParentMenu({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    getHealth()
      .then((h) => {
        setManaged(!!h.managed);
        bootRef.current = h.boot ?? null;
      })
      .catch(() => {});
  }, []);
  ...
  const verify = async (candidate: string) => {
    try {
      ...
      if (r.ok) {
        pinRef.current = candidate;
        setManaged(!!r.managed);
        setStep('menu');
        return;
      }
    } catch {
      /* fall through to the wrong-PIN shake */
    }
    setShake(true);
    setTimeout(() => {
      setShake(false);
  ...
  const doUpdate = async () => {
    ...
    let r;
    try {
      r = await kioskUpdate(pinRef.current);
    } catch {
    ...
    if (r.status === 'up-to-date') {
      setMsg(`Already up to date${r.rev ? ` (${r.rev})` : ''}.`);
      setStep('uptodate');
      return;
    }
  ...
  const pollForRestart = () => {
    const t0 = Date.now();
    const tick = async () => {
      if (Date.now() - t0 > UPDATE_TIMEOUT_MS) {
        setMsg('Update is taking too long — check the device.');
        setStep('error');
        return;
      }
      try {
        const h = await getHealth();
        if (bootRef.current != null && h.boot != null && h.boot !== bootRef.current) {
    ...
    <div className="pm-overlay" onPointerDown={(e) => e.stopPropagation()}>
      {step === 'pin' && (
        <div className={`pm-card${shake ? ' pm-shake' : ''}`}>
          ...
          <div className="pm-pad">
            ...
            <button className="pm-back" onClick={back} aria-label="Delete">
              ⌫
            </button>
          </div>
          <p className="pm-hint">
            Enter the 4-digit PIN. Change it in the parent console → Settings.
          </p>
        </div>
      )}
      ...
      {step === 'menu' && (
        <div className="pm-card">
          ...
          <div className="pm-menu">
            {managed ? (
              <button className="pm-act" onClick={doUpdate}>
                <b>⟳ Update</b>
                <small>Get the latest version</small>
              </button>
            ) : (
              <button className="pm-act" disabled>
                <b>⟳ Update</b>
                <small>Available on the device only</small>
              </button>
            )}
            <button className="pm-act" onClick={() => window.location.reload()}>
              <b>↻ Reload</b>
              <small>Refresh the screen</small>
            </button>
          </div>
        </div>
      )}
      ...
      {step === 'updating' && (
        <div className="pm-card pm-busy">
          ...
          <h2>Updating…</h2>
          <p>
            Please keep the device on. This can take a few minutes — it will restart on its own.
          </p>
        </div>
      )}
      ...
      {step === 'uptodate' && (
        <div className="pm-card">
          <h2>✓ Up to date</h2>
          <p>{msg}</p>
          <button className="pm-act pm-solo" onClick={onClose}>
            Done
          </button>
        </div>
      )}
