...
export interface SceneIconData {
  viewBox: string;
  shapes: SceneShape[];
}
...
export type DeclBlock =
  ...
  | {
      type: 'scene';
      layout: 'orbit' | 'map' | 'cycle';
      nodes: SceneNode[];
      center?: SceneNode;
      backdrop?: 'body' | 'plant' | 'globe';
      caption?: string;
    }
  | {
      type: 'image';
...
export interface ContentTypeManifest {
  ...
  createForm: CreateFormField[];
  triggersHelp: string | null;
  authorable: boolean;
  enabled: boolean;
}
...
export interface TurnResponse {
  kind: 'chat' | 'artifact' | 'timer' | 'reminder';
  reply: string;
  artifactId?: string;
  artifact?: Artifact;
  blocked?: boolean;
...
export interface RichnessConfig {
  selected: string;
  default: string;
  dailyCap: number;
  tiers: RichnessTier[];
...
export interface WakeConfig {
  enabled: boolean;
  phrase: string;
  phrases: { key: string; label: string }[];
}
export interface AdminConfig {
  systemPrompt: string;
  defaultSystemPrompt: string;
  chatSystemPrompt: string;
  defaultChatSystemPrompt: string;
  readingSystemPrompt: string;
  defaultReadingSystemPrompt: string;
...
export interface EvalRunInfo {
  ...
  summary: EvalSummary;
  prevSummary: EvalSummary | null;
  prevWhen: number | null;
}
export interface EvalsResponse {
  ...
  allTime: EvalSummary;
  latestRun: EvalRunInfo | null;
  job: EvalJob;
  live: boolean;
  promptChangedAt?: number | null;
}
