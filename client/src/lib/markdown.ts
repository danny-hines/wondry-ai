...
export function mdToHtml(src: string): string {
  ...
  for (const raw of lines) {
    ...
    if ((m = line.match(/^(#{1,6})\s+(.*)$/))) { closeList(); const n = m[1].length; html += `<h${n}>${mdInline(m[2])}</h${n}>`; continue; }
...
export function stripMarkdown(s: string): string {
  return String(s)
    ...
    .replace(/^\s{0,3}#{1,6}\s+/gm, '').replace(/(\*\*|__)(.*?)\1/g, '$2').replace(/(\*|_)(.*?)\1/g, '$2')
    .replace(/^\s*[-*+]\s+/gm, '').replace(/^\s*\d+\.\s+/gm, '').replace(/^\s*>\s?/gm, '')
    // Em/en dashes (and "--") read oddly aloud; turn them into a comma pause.
    .replace(/\s*(?:—|–|--+)\s*/g, ', ')
    .replace(/[ \t]{2,}/g, ' ').replace(/\n{2,}/g, '. ').replace(/\s+([.,])/g, '$1').replace(/,\s*,/g, ',').trim();
}
