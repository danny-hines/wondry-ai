...
export const getProfiles = () =>
  fetch('/api/profiles')
    .then(j<{ profiles: Profile[] }>)
    .then((d) => d.profiles);
export const getTray = (profileId: string) =>
  fetch(`/api/artifacts?profileId=${profileId}`).then(j<TrayResponse>);
...
export const markEngagement = (
  id: string,
  kind: 'seen' | 'opened' | 'finished',
  profileId: string,
) =>
  fetch(`/api/artifacts/${id}/${kind}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ profileId }),
  });
export const ttsArrayBuffer = async (
  text: string,
  profileId?: string,
  voice?: string,
  ...
  try {
    const r = await fetch('/api/tts', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ text, profileId, voice }),
    });
...
export const getVoices = () =>
  fetch('/api/voices')
    .then(j<{ voices: string[]; available: boolean; browserVoice?: string }>)
    .catch(() => ({ voices: [] as string[], available: false, browserVoice: undefined }));
...
export const getAudioConfig = () =>
  fetch('/api/audio')
    .then(j<{ warmHz: number; warmGain: number }>)
    .catch(() => ({ warmHz: 0, warmGain: 0.02 }));
export const setAudioConfig = (warmHz: number, warmGain: number) =>
  fetch('/api/audio', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ warmHz, warmGain }),
  }).then(j<{ warmHz: number; warmGain: number }>);
...
export const getTimers = () =>
  fetch('/api/timers')
    .then(j<{ timers: ScheduleItem[] }>)
    .catch(() => ({ timers: [] as ScheduleItem[] }));
// Console: every active schedule (timers + reminders).
export const getSchedules = () =>
  fetch('/api/schedules')
    .then(j<{ schedules: ScheduleItem[] }>)
    .catch(() => ({ schedules: [] as ScheduleItem[] }));
export const createTimer = (
  durationMs: number,
  label?: string | null,
  createdBy: 'voice' | 'parent' = 'voice',
) =>
  fetch('/api/timers', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ durationMs, label, createdBy }),
  }).then(j<{ schedule: ScheduleItem }>);
...
export const createReminder = (atLocal: string, message?: string | null, label?: string | null) =>
  fetch('/api/reminders', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ atLocal, message, label, createdBy: 'parent' }),
  }).then(j<{ schedule: ScheduleItem; error?: string }>);
...
export const getHealth = () =>
  fetch('/api/health').then(
    j<{ ok: boolean; boot?: number; managed?: boolean; liveGeneration?: boolean; tts?: boolean }>,
  );
export const kioskVerifyPin = (pin: string) =>
  ...
  }).then(j<{ ok: boolean; managed: boolean }>);
export const kioskUpdate = (pin: string) =>
  fetch('/api/kiosk/update', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ pin }),
  }).then(
    j<{
      status: 'up-to-date' | 'updating' | 'unmanaged' | 'bad-pin' | 'error';
      rev?: string;
      error?: string;
    }>,
  );
...
export const getContentTypes = () =>
  fetch('/api/content-types')
    .then(j<{ types: ContentTypeManifest[] }>)
    .catch(() => ({ types: [] as ContentTypeManifest[] }));
// Record a scored interaction; the server's content type interprets the event.
export const postContentEvent = (
  id: string,
  profileId: string,
  event: ReadingAttempt | Record<string, unknown>,
) =>
  fetch(`/api/content/${id}/event`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
export const serverTranscribe = async (
  audio?: Blob,
...
export class AdminApi {
  ...
  private h() {
    return { 'content-type': 'application/json', 'x-admin-password': this.pw };
  }
  private req(path: string, opts: RequestInit = {}) {
    return fetch('/api/admin' + path, {
      ...opts,
      headers: { ...this.h(), ...(opts.headers || {}) },
    });
  }
  async ok(): Promise<boolean> {
    return (await this.req('/config')).ok;
  }
  ...
  saveConfig = (body: {
    ...
    timezone?: string;
    promptAuthor?: string;
  }) => this.req('/config', { method: 'POST', body: JSON.stringify(body) });
  promptHistory = (key: string) =>
    this.req(`/prompt-history?key=${key}`).then(j<{ versions: PromptVersion[] }>);
  ...
  setAudience = (id: string, profileId: string, on: boolean) =>
    this.req(`/artifacts/${id}/audience`, {
      method: 'POST',
      body: JSON.stringify({ profileId, on }),
    });
  deleteArtifact = (id: string) => this.req(`/artifacts/${id}/delete`, { method: 'POST' });
  author = (topic: string) =>
    this.req('/author', { method: 'POST', body: JSON.stringify({ topic }) }).then(
      j<{ ok: boolean; artifactId: string }>,
    );
  authorReading = (body: { interest?: string; level?: number; profileId?: string }) =>
    this.req('/author-reading', { method: 'POST', body: JSON.stringify(body) }).then(
      j<{ ok: boolean; artifactId: string }>,
    );
  createContent = (body: {
    typeId: string;
    ...
    this.req('/content', { method: 'POST', body: JSON.stringify(body) }).then(
      j<{ ok: boolean; artifactId: string }>,
    );
  ...
  runEvals = (body: { mode?: 'benchmark' | 'live'; kind?: string; reeval?: boolean }) =>
    this.req('/evals/run', { method: 'POST', body: JSON.stringify(body) }).then(
      j<{ started?: boolean; error?: string }>,
    );
  suggestPrompt = (kind: string) =>
    this.req('/evals/suggest', { method: 'POST', body: JSON.stringify({ kind }) }).then(
      j<EvalSuggestion>,
    );
  profiles = () => this.req('/profiles').then(j<{ profiles: Profile[] }>);
  saveProfile = (p: Partial<Profile> & { disabledTypes?: string[] }) =>
    this.req('/profiles', { method: 'POST', body: JSON.stringify(p) });
  contentTypes = () => this.req('/content-types').then(j<{ types: ContentTypeManifest[] }>);
  setContentTypeEnabled = (id: string, enabled: boolean) =>
    this.req(`/content-types/${id}`, { method: 'POST', body: JSON.stringify({ enabled }) });
  deleteProfile = (id: string) => this.req(`/profiles/${id}/delete`, { method: 'POST' });
  faces = () => this.req('/faces').then(j<FacesResponse>);
  assignFace = (clusterId: string, profileId: string) =>
    this.req(`/faces/clusters/${clusterId}/assign`, {
      method: 'POST',
      body: JSON.stringify({ profileId }),
    });
  faceCluster = (clusterId: string, action: 'ignore' | 'unassign' | 'delete') =>
    this.req(`/faces/clusters/${clusterId}/${action}`, { method: 'POST' });
}
