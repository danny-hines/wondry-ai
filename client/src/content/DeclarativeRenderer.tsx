...
export default function DeclarativeRenderer({ artifactId, profile, speak, setMood }: ContentRendererProps) {
  ...
  useEffect(() => {
    let live = true;
    getContent<DeclarativeDoc>(artifactId).then((d) => { if (live) setDoc(d); }).catch(() => { if (live) setErr(true); });
    return () => { live = false; };
  }, [artifactId]);
  useEffect(() => { setMood('idle'); }, [setMood]);

  const finish = () => {
    if (!finishedRef.current && pid) { finishedRef.current = true; markEngagement(artifactId, 'finished', pid); }
    setDone(true);
    say(`Awesome work${profile?.name ? ', ' + profile.name : ''}!`);
  };

  if (err) return <div className="decl"><div className="decl-msg">Couldn't load this. Try again!</div></div>;
  if (!doc) return <div className="decl"><div className="decl-msg">✨ Getting it ready…</div></div>;

  return (
    <div className="decl" style={{ ['--user' as any]: profile?.color || '#0ea5e9' }}>
      <h1 className="decl-title">{doc.emoji} {doc.title}</h1>
      {doc.blocks.map((b, i) => <Block key={i} block={b} say={say} />)}
      <div className="decl-foot">
        <button className="rbtn primary" onClick={finish}>{done ? 'All done! 🎉' : "I'm finished! 🎉"}</button>
      </div>
    </div>
  );
}
...
function Block({ block, say }: { block: DeclBlock; say: (t: string) => void }) {
  switch (block.type) {
    case 'heading': return <h2 className="decl-h">{block.text}</h2>;
    case 'text': return <p className="decl-text" onClick={() => say(block.text)} title="Tap to hear">{block.text}</p>;
    case 'image': return <Image block={block} />;
    case 'flashcards': return <Flashcards cards={block.cards} say={say} />;
    case 'scene': return <Scene block={block} say={say} />;
    case 'quiz': return <Quiz block={block} say={say} />;
    default: return null;
  }
}
  ...
  return (
    <figure className="decl-figure">
      ...
      {(block.caption || block.credit) && (
        <figcaption>{block.caption}{block.credit ? <span className="decl-credit"> · {block.credit}</span> : null}</figcaption>
      )}
    </figure>
  );
}

function Flashcards({ cards, say }: { cards: { front: string; back: string; hint?: string }[]; say: (t: string) => void }) {
  ...
  return (
    <div className="decl-cards">
      {cards.map((c, i) => {
        ...
        return (
          <div key={i} className={`fcard${isBack ? ' flipped' : ''}`} onClick={() => setFlipped((f) => ({ ...f, [i]: !f[i] }))}>
            <div className="fcard-face">
              ...
              <button className="fcard-hear" title="Tap to hear" onClick={(e) => { e.stopPropagation(); say(face); }}>🔊</button>
            </div>
          </div>
        );
      })}
...
function Scene({ block, say }: { block: SceneBlock; say: (t: string) => void }) {
  ...
  const jump = (d: number) => setFocus((f) => {
    const n = focusable.length, cur = f == null ? 0 : f;
    return (((cur + d) % n) + n) % n;
  });
      ...
      <div className="scene-stage">
        ...
        {node && (
          <div className="scene-detail">
            <button className="scene-x" onClick={() => setFocus(null)} title="Back to the map">✕</button>
            <div className="scene-detail-emoji">{node.emoji}</div>
            <h3 className="scene-detail-label">{node.label}</h3>
            {node.blurb && <p className="scene-detail-blurb" onClick={() => say(node.blurb!)} title="Tap to hear">{node.blurb}</p>}
            {!!node.facts?.length && (
              <ul className="scene-facts">
                {node.facts.map((f, i) => (
                  <li key={i} onClick={() => say(f)} title="Tap to hear"><span className="scene-fact-hear">🔊</span> {f}</li>
                ))}
              </ul>
            )}
            <div className="scene-nav">
              <button className="rbtn" onClick={() => jump(-1)}>‹ Prev</button>
              <span className="scene-nav-pos">{focus! + 1} / {focusable.length}</span>
              <button className="rbtn" onClick={() => jump(1)}>Next ›</button>
            </div>
          </div>
        )}
      </div>
...
function NodeButton({ nd, on, onClick, disc = 58 }: { nd: SceneNode; on: boolean; onClick: () => void; disc?: number }) {
  const accent = nd.color || undefined;
  return (
    <button type="button" className={`node-btn${on ? ' on' : ''}`} onClick={onClick}>
      <span className="node-disc" style={{ width: disc, height: disc, fontSize: disc * 0.5, ...(accent ? { ['--node' as any]: accent } : {}) }}>
...
function SceneIcon({ icon, size }: { icon: SceneIconData; size: number }) {
  return (
    <svg className="node-icon" width={size} height={size} viewBox={icon.viewBox} aria-hidden="true">
      {icon.shapes.map((s, i) => <Shape key={i} s={s} />)}
    </svg>
  );
}
function Shape({ s }: { s: SceneShape }) {
  ...
  const common = {
    ...
    stroke: s.stroke, strokeWidth: s.strokeWidth, strokeLinecap: s.strokeLinecap as any,
    strokeLinejoin: s.strokeLinejoin as any, opacity: s.opacity,
  };
  switch (s.type) {
    case 'path': return <path {...common} d={s.d} />;
    case 'circle': return <circle {...common} cx={s.cx} cy={s.cy} r={s.r} />;
    case 'ellipse': return <ellipse {...common} cx={s.cx} cy={s.cy} rx={s.rx} ry={s.ry} />;
    case 'rect': return <rect {...common} x={s.x} y={s.y} width={s.width} height={s.height} rx={s.rx} ry={s.ry} />;
    case 'line': return <line {...common} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} />;
    case 'polygon': return <polygon {...common} points={s.points} />;
    case 'polyline': return <polyline {...common} points={s.points} />;
    default: return null;
  }
}

function OrbitLayout({ block, paused, focus, onFocus }: { block: SceneBlock; paused: boolean; focus: number | null; onFocus: (i: number) => void }) {
  const { nodes } = block, n = nodes.length, hasCenter = !!block.center;
  return (
    <div className="orbit">
      {hasCenter && (
      ...
      {nodes.map((nd, i) => {
        ...
        const anim = { animationDuration: `${dur}s`, animationDelay: `${delay}s`, animationPlayState: paused ? 'paused' : 'running' } as const;
        return (
          <div key={i} className="orbit-ring" style={{ width: `${ringPct}%`, height: `${ringPct}%`, ...anim }}>
            <div className="orbit-node" style={anim}>
              <NodeButton nd={nd} on={focus === fIdx} onClick={() => onFocus(fIdx)} disc={Math.round(46 * (nd.size || 1))} />
            </div>
          </div>
        );
      })}
...
function fitNodes(nodes: SceneNode[]): { x: number; y: number }[] {
  const xs = nodes.map((n) => n.x ?? 50), ys = nodes.map((n) => n.y ?? 50);
  const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
  const span = Math.max(maxX - minX, maxY - minY, 1);
  const scale = Math.min(2.2, 76 / span);                  // fill ~76% of the stage, don't over-zoom
  const cx = (minX + maxX) / 2, cy = (minY + maxY) / 2;
  return nodes.map((n) => ({ x: 50 + ((n.x ?? 50) - cx) * scale, y: 50 + ((n.y ?? 50) - cy) * scale }));
}

function MapLayout({ block, focus, onFocus }: { block: SceneBlock; focus: number | null; onFocus: (i: number) => void }) {
  const pts = block.backdrop ? block.nodes.map((n) => ({ x: n.x ?? 50, y: n.y ?? 50 })) : fitNodes(block.nodes);
  return (
    <div className="map">
...
function Backdrop({ kind }: { kind: 'body' | 'plant' | 'globe' }) {
  return (
    <svg className="scene-backdrop" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      {kind === 'body' && (
...
function CycleLayout({ block, focus, onFocus }: { block: SceneBlock; focus: number | null; onFocus: (i: number) => void }) {
  const { nodes } = block, n = nodes.length, R = 38;
  const pos = (i: number) => {
  ...
  return (
    <div className="cycle">
      <svg className="cycle-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
...
function Quiz({ block, say }: { block: Extract<DeclBlock, { type: 'quiz' }>; say: (t: string) => void }) {
  ...
  return (
    <div className="decl-quiz">
      <p className="decl-q" onClick={() => say(block.question)} title="Tap to hear">🧠 {block.question}</p>
